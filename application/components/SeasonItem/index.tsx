  ...
  return (
    <View>
      <View
        style={[
          styles.container,
          showEpisodeList ? styles.activeTab : styles.inactiveTab,
        ]}>
        <TouchableOpacity
          onPress={() => {
          ...
          <View style={styles.seasonButton}>
            <Text
              style={[
                styles.title,
                showEpisodeList ? styles.titleActive : styles.titleInactive,
              ]}>
              Season {props.number}
            </Text>
            <Icon
              name={icon}
              size={20}
              color={showEpisodeList ? activeTitleColor : inactiveColor}
            />
          </View>
        </TouchableOpacity>
      </View>
      {showEpisodeList ? (
        <View style={[styles.container, styles.inactiveTab]}>
          {props.episodes.map((episode: Episode, index: Number) => {
            return <EpisodeItem key={index.toString()} episode={episode} />;
          })}
        </View>
      ) : (
        ''
